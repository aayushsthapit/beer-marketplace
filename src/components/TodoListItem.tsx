...
function TodoListItem(props: TodoListItemProps) {
  ...
  return (
    <Accordion>
      {/* Todo title content */}
      <AccordionSummary id="panel1a-header" expandIcon={<ExpandMoreIcon />} aria-controls="panel1a-content">
        <Typography>
          <Checkbox
            checked={status === Status.COMPLETED}
            onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
              const status = event.target.checked ? Status.COMPLETED : Status.PENDING;
      ...
      <AccordionDetails>
        <Typography>
          {subtasksList.map((subtask) => (
            <SubtasksListItem subtask={subtask} key={subtask.id} updateSubtask={updateSubtask} />
          ))}
        </Typography>
